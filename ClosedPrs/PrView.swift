...
class PrViewController: UIViewController, AnyView , UITableViewDelegate , UITableViewDataSource{
    ...
    private let pageLimit = 29
    private var currentLastId: Int? = nil
    
    private var closedPrs = [ClosedPR]() {
        didSet {
    ...
    func update(with closedPrs: ClosedPrs) {
        DispatchQueue.main.async {
            self.closedPrs.append(contentsOf: closedPrs)
            
            self.currentLastId = closedPrs.last?.id
    ...
    override func viewDidLoad() {
        ...
        self.view.pinAllEdges(viewToPin: errLabel, insets: [0,0,0,0])
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        guard !closedPrs.isEmpty else { return }
        
        if section == .loader {
            print("load new data..")
            self.presenter?.fetchNextBatch(perPage: 30, sinceId: currentLastId)
        }
    }
    // 11
    private func hideBottomLoader() {
        DispatchQueue.main.async {
            let lastListIndexPath = IndexPath(row: self.closedPrs.count - 1, section: TableSection.prList.rawValue)
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        case .loader:
            return closedPrs.count >= pageLimit ? 1 : 0
        }
    }
